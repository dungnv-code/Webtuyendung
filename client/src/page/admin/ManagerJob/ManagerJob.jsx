...
const ManagerJob = () => {
    ...
    const hanleCreateJob = async () => {
        ...
        if (!inputValue.trim()) {
            newError.inputValue = "Vui lòng nhập tên công việc";
        }
    ...
    const hanleUpdateJob = async () => {
        ...
        if (!currentIndex.trim()) {
            newError.inputValue = "Vui lòng chọn công việc muốn sửa!";
        }
        setError(newError);
        if (Object.keys(newError).length === 0) {
            try {
    ...
    const hanleSearch = async () => {
        const newError = {};
        if (!inputValue.trim()) {
            newError.inputValue = "Vui lòng nhập tên công việc bạn muốn tìm";
        }
        setError(newError);
        if (Object.keys(newError).length === 0) {
            try {
                        ...
                        <tbody>
                            {listJob?.map((job, index) => (
                                <tr key={job._id}>
                                    ...
                                    <td>{job.title}</td>
                                    <td>{job.slug}</td>
                                    <td className="text-primary fs-5" onClick={() => { setCurrentIndex(job._id), setInputValue(job.title) }} role="button">
                                        <EditTwoTone />
                                    </td>
