...
const ManagerLevel = () => {
    ...
    const hanleCreateJob = async () => {
        ...
        if (!inputValue.trim()) {
            newError.inputValue = "Vui lòng nhập tên cấp bậc";
        }
    ...
    const hanleUpdateJob = async () => {
        ...
        if (!currentIndex.trim()) {
            newError.inputValue = "Vui lòng chọn cấp bậc muốn sửa!";
        }
        setError(newError);
        if (Object.keys(newError).length === 0) {
            try {
                ...
                setInputValue("");
                setCurrentPage(1);
                toast.success("Sửa cấp bậc thành công!")
                setLoaddata(!loaddata)
            } catch (error) {
    ...
    const hanleSearch = async () => {
        const newError = {};
        if (!inputValue.trim()) {
            newError.inputValue = "Vui lòng nhập tên cấp bậc bạn muốn tìm!";
        }
        setError(newError);
        if (Object.keys(newError).length === 0) {
            try {
                        ...
                        <tbody>
                            {listJob?.map((job, index) => (
                                <tr key={job._id}>
                                    ...
                                    <td>{job.nameLevel}</td>
                                    <td>{job.slug}</td>
                                    <td className="text-primary fs-5" onClick={() => { setCurrentIndex(job._id), setInputValue(job.nameLevel) }} role="button">
                                        <EditTwoTone />
                                    </td>
